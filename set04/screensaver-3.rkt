...
;; screen saver #3
;; Similar to screen saver #2, except multiple rectangles and velocity set up
...
(define RECT_IN_LEFT_TOP_CORNER_NON_MOVING (make-rect 60 50 0 0 false 0 0))
(define RECT_NEAR_RIGHT_WAL_NON_MOVING (make-rect 350 100 -5 0 false 0 0))
(define RECT_IN_CENTER_SELECTED (make-rect 200 100 10 10 true 5 5))
...
;; EXAMPLES/CONSTANTS for tests:
(define TEST_LOR
  (list (make-rect 200 100 -12 20 false 0 0)
        (make-rect 200 200 23 -14 false 0 0)))
(define TEST_LOR_AFTER_TIC
  (list (make-rect 188 120 -12 20 false 0 0)
...
;; rects - is a list of created Rectangles
...
;; EXAMPLES/CONSTANTS for tests:
(define TEST_WORLD (make-world TEST_LOR false))
(define TEST_WORLD_AFTER_TIC (make-world TEST_LOR_AFTER_TIC false))
(define TEST_WORLD_BEFORE_KEY_EVENT (make-world TEST_LOR_AFTER_KEY_EVENT true))
(define TEST_WORLD_AFTER_KEY_EVENT (make-world TEST_LOR_AFTER_KEY_EVENT false))
...
(begin-for-test
  (check-equal?
   (lor-after-tick
    (list (make-rect 200 100 -12 20 false 0 0)
          (make-rect 200 200 0 0 false 0 0)
          (make-rect 200 200 30 -5 true 0 0)))
    (list (make-rect 188 120 -12 20 false 0 0)
          (make-rect 200 200 0 0 false 0 0)
          (make-rect 200 200 30 -5 true 0 0))
...
(begin-for-test
  ...
  (check-equal? (world-after-key-event (initial-world 7) "n")
                (make-world (cons (new-centered-rectangle 7) empty) true)
...
;; EXAMPLE:
;; (world-after-new-key (make-world empty true)) =>
;; (make-world (make-rect HALF_CANVAS_WIDTH HALF_CANVAS_HIGH 0 0 false 0 0) true)
...
;; (make-rect HALF_CANVAS_WIDTH HALF_CANVAS_HIGH 0 0 false 0 0)
...
(begin-for-test
  (check-equal? (world-after-rect-key
                 (make-world (list (make-rect 50 50 0 0 true 0 0)) false) "left")
                 (make-world (list (make-rect 50 50 -2 0 true 0 0)) false)
...
(begin-for-test
  (check-equal? (key-event-for-each-rect
                 (list (make-rect 50 50 0 0 true 0 0)) "up")
                 (list (make-rect 50 50 0 -2 true 0 0))
...
;; TESTS\EXAMPLES:
(begin-for-test
  (check-equal? (rect-after-key-event (make-rect 50 50 0 0 false 0 0) "left")
                (make-rect 50 50 0 0 false 0 0)
                "Unselected rectangle should not change")
  (check-equal? (rect-after-key-event (make-rect 50 50 0 0 true 0 0) "left")
                (make-rect 50 50 -2 0 true 0 0)
...
(begin-for-test
  (check-equal?
   (lor-after-mouse-event
    (cons (make-rect 200 100 10 20 false 0 0) empty) 200 100 "button-down")
   (cons (make-rect 200 100 10 20 true 0 0) empty)
   "Rectangle from ListOfRectangle should be selected")
  (check-equal?
   (lor-after-mouse-event
    (cons (make-rect 200 100 10 20 false 0 0) empty) 20 10 "button-down")
   (cons (make-rect 200 100 10 20 false 0 0) empty)
