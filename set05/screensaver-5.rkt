...
;; screen saver #5
;; Similar to screen saver #4, except HOFs are used
...
;; -- > INTERP: for right and bottom walls
;; -- < INTERP: for left and top walls
;; EXAMPLES/CONSTANTS for tests:
(define POSITIVE >)
(define NEGATIVE <)
...
;; A Rectangle is a (make-rect NonNegInt NonNegInt Int Int Boolean Int Int Boolean)
;; x and y are coordinates of the center of rectangle
...
;; pen-down? describes whether or not the rectangle's pen is up (false) or down (true)
...
(define (lor-after-tick lor)
  (map
   (lambda (rect) (rect-after-tick rect))
   lor))
...
(define (rect-after-tick rct)
  (if (rect-selected? rct)
      rct
      (calculate-rect-x (calculate-rect-y rct))))
...
;; calculate-rect-x : Rectangle -> Rectangle
;; GIVEN: Rectangle
...
(define (calculate-rect-x rct)
  (if (> (rect-vx rct) 0)
      (move-x-direction rct rect-hits-wall POSITIVE CANVAS_COUNTABLE_WIDTH)
      (move-x-direction rct rect-hits-wall NEGATIVE ZERO_COORDINATE_X)))

;; TESTS/EXAMPLES: 
(begin-for-test
  (check-equal?
   (calculate-rect-x (make-rect 360 265 30 20 false 0 0 false))
   (make-rect 370 265 -30 20 false 0 0 false)
   "After right wall hit x will equal 370 and x velocity will change from 30 to -30")
  (check-equal?
   (calculate-rect-x (make-rect 300 200 30 20 false 0 0 false))
   (make-rect 330 200 30 20 false 0 0 false)
   "Without right wall hit, x will increase by 30 and will equal 330")
  (check-equal?
   (calculate-rect-x (make-rect 40 35 -30 -20 false 0 0 false))
...
;; move-x-direction : Rectangle (NonNegInt Int NonNegInt -> Boolean) Direction Integer
;  -> Rectangle
;; GIVEN: Rectangle, verification function, Direction and destination wall
;; RETURNS: Rectangle, with updated x-coordinate based on given direction.
;  In case of wall hit, x-coordinate will be placed near the wall,
;  and velocity vx will change to opposite number.
;; STRATEGY: Use template for Rectangle on rct
(define (move-x-direction rct fn direction wall)
  (if (fn (rect-x rct) (rect-vx rct) direction wall)
      (make-rect wall
                 (rect-y rct)
                 (- (rect-vx rct))
                 (rect-vy rct)
...
(begin-for-test
  (check-equal?
   (move-x-direction (make-rect 340 265 30 20 false 0 0 true)
                     rect-hits-wall >
                     CANVAS_COUNTABLE_WIDTH)
   (make-rect 370 265 30 20 false 0 0 true)
   "X will change 340 -> 370")
  (check-equal?
   (move-x-direction (make-rect 360 200 30 20 false 20 20 true)
                     rect-hits-wall >
                     CANVAS_COUNTABLE_WIDTH)
   (make-rect 370 200 -30 20 false 20 20 true)
   "X will change 360 -> 370. Because of wall hit, vx will change to opposite")
  (check-equal?
   (move-x-direction (make-rect 340 265 -30 20 false 0 0 true)
                     rect-hits-wall <
                     ZERO_COORDINATE_X)
   (make-rect 310 265 -30 20 false 0 0 true)
   "X will change 340 -> 310")
  (check-equal?
   (move-x-direction (make-rect 40 200 -30 20 false 20 20 true)
                     rect-hits-wall <
                     ZERO_COORDINATE_X)
   (make-rect 30 200 30 20 false 20 20 true)
   "X will change 40 -> 30. Because of wall hit, vx will change to opposite"))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; rect-hits-wall : NonNegInt Int Direction NonNegInt -> Boolean
;; Given: appropriate coordinate, appropriate velocity , Direction and wall size 
;; Returns: true if coordinate + velocity hits the wall, else - false
(define (rect-hits-wall coord vel direction wall)
  (direction (+ coord vel) wall))

;; TESTS/EXAMPLES:
(begin-for-test
  (check-equal?
   (rect-hits-wall 10 20 > 100)
   false
   "Given wall is to far (false)")
  (check-equal?
   (rect-hits-wall 90 20 > 100)
   true
   "Given wall is near enough for hit (true)")
  (check-equal?
   (rect-hits-wall 100 -20 < 0)
   false
   "Given wall is to far (false)")
  (check-equal?
   (rect-hits-wall 10 -20 < 0)
   true
   "Given wall is near enough for hit (true)"))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; calculate-rect-y : Rectangle -> Rectangle
;; GIVEN: Rectangle
;; RETURNS: given Rectangle, except that y-coordinate has changed.
;  In case of wall hit, velocity vy will also change.
;; STRATEGY: Use more general function
(define (calculate-rect-y rct)
  (if (> (rect-vy rct) 0)
      (move-y-direction rct rect-hits-wall > CANVAS_COUNTABLE_HIGH)
      (move-y-direction rct rect-hits-wall < ZERO_COORDINATE_Y)))


;; TESTS/EXAMPLES: 
(begin-for-test
  (check-equal?
   (calculate-rect-y (make-rect 360 265 30 20 false 0 0 false))
   (make-rect 360 275 30 -20 false 0 0 false)
   "After bottom wall hit y will equal 275 and y velocity will change from 20 to -20")
  (check-equal?
   (calculate-rect-y (make-rect 300 200 30 20 false 0 0 false))
   (make-rect 300 220 30 20 false 0 0 false)
   "Without bottom wall hit, y will increase by 20 and will equal 220")
  (check-equal?
   (calculate-rect-y (make-rect 360 35 30 -20 false 0 0 false))
...
;; move-y-direction : Rectangle (NonNegInt Int NonNegInt -> Boolean) Direction Integer
;  -> Rectangle
;; GIVEN: Rectangle, verification function and destination wall
;; RETURNS: Rectangle, with updated y-coordinate based on given direction.
;  In case of wall hit, y-coordinate will be placed near the wall,
;  and velocity vy will change to opposite number.
;; STRATEGY: Use template for Rectangle on rct
(define (move-y-direction rct fn direction wall)
  (if (fn (rect-y rct) (rect-vy rct) direction wall)
...
(begin-for-test
  (check-equal?
   (move-y-direction (make-rect 340 265 -30 20 false 0 0 true)
                     rect-hits-wall >
                     CANVAS_COUNTABLE_HIGH)
   (make-rect 340 275 -30 -20 false 0 0 true)
   "y will change 265 -> 275. Because of wall hit, vy will change to opposite")
  (check-equal?
   (move-y-direction (make-rect 40 200 -30 20 false 20 20 true)
                     rect-hits-wall >
                     CANVAS_COUNTABLE_HIGH)
   (make-rect 40 220 -30 20 false 20 20 true)
   "y will change 200 -> 220")
  (check-equal?
   (move-y-direction (make-rect 340 265 -30 -20 false 0 0 true)
                         rect-hits-wall <
                         ZERO_COORDINATE_Y)
   (make-rect 340 245 -30 -20 false 0 0 true)
   "y will change 265 -> 245")
  (check-equal?
   (move-y-direction (make-rect 40 30 -30 -20 false 20 20 true)
                         rect-hits-wall <
                         ZERO_COORDINATE_Y)
   (make-rect 40 25 -30 20 false 20 20 true)
   "y will change 30 -> 25. Because of wall hit, vy will change to opposite"))
...
(define (dots-after-tick lor lod)
  (cond
       ...
       [else (if (allowed-to-create-dot? (first lor))
                 (cons (new-dot-after-tick (first lor))
...
;; new-dot-after-tick : Rectangle -> Dot
;; GIVEN: a Rectangle 
;; RETURNS: a Dot with the same center as a given Rectangle
;; STRATEGY: Use template for Rectangle on rct
(define (new-dot-after-tick rct)
  (make-dot (rect-x rct) (rect-y rct)))

;; TEST/EXAMPLE: 
(begin-for-test
  (check-equal?
   (new-dot-after-tick (new-rectangle 50 100 -10 -20))
   (make-dot 50 100)
...
(define (rect-to-scene lor lod)
  (foldr draw-rect (dot-to-scene lod) lor))
...
(define (rect-img rct)
  (if (rect-selected? rct)
      ...
      (rect-img-unselected (rect-vx rct)
                           (rect-vy rct))))
...
;; RETURNS: Rectangle Image with: red color and given velocity as text in the center
;; STRATEGY: Use more general function
(define (red-rect vx vy)
  (general-rect-img vx vy SELECTED_RECT_COLOR))
...
;; GIVEN: x- and y- velocity of rectangle
;; RETURNS: Rectangle Image with: blue color, and given velocity as text in the center
;; STRATEGY: Use more general function
(define (rect-img-unselected vx vy)
  (general-rect-img vx vy RECT_COLOR))
...
;; general-rect-img : Int Int Color-> Image
;; GIVEN: x- and y- velocity of rectangle, and rectangle color
;; RETURNS: Rectangle Image with: given color, and given velocity as text in the center
;; STRATEGY: Combine simpler functions
(define (general-rect-img vx vy color)
  (overlay
   (text (string-append "(" (number->string vx) ", " (number->string vy) ")")
         RECT_FONT color)
   (rectangle RECT_WIDTH RECT_HIGH RECT_STYLE color)))

;; TESTS/EXAMPLES: in red-rect and rect-img-unselected
...
(define (dot-to-scene lod)
  (foldr draw-dot EMPTY_CANVAS lod))
...
(define (selected-rect-after-key-event rct kev)
  (cond
    [(string=? kev LEFT_KEY) (rect-change-velocity-x rct MINUS)]
    [(string=? kev RIGHT_KEY) (rect-change-velocity-x rct PLUS)]
    [(string=? kev UP_KEY) (rect-change-velocity-y rct MINUS)]
    [(string=? kev DOWN_KEY) (rect-change-velocity-y rct PLUS)]
    [(string=? kev PEN_DOWN_KEY) (rect-pen-change rct true)]
    [(string=? kev PEN_UP_KEY) (rect-pen-change rct false)]
    [else rct]))
...
;; rect-change-velocity-x : Rectangle Sign-> Rectangle
;; rect-change-velocity-y : Rectangle Sign -> Rectangl
;; GIVEN: Rectangle rct and a Sign
;; RETURNS: Given rectangle with velocity changed according to given sign
;; STRATEGY: Use template for Rectangle on rct
(define (rect-change-velocity-x rct sign)
  (make-rect
   (rect-x rct)
   (rect-y rct)
   (sign (rect-vx rct) INCREASE_BY)
   (rect-vy rct)
   (rect-selected? rct)
   (rect-sel-x rct)
   (rect-sel-y rct)
...
(define (rect-change-velocity-y rct sign)
  (make-rect
   (rect-x rct)
   (rect-y rct)
   (rect-vx rct)
   (sign (rect-vy rct) INCREASE_BY)
   (rect-selected? rct)
   (rect-sel-x rct)
   (rect-sel-y rct)
   (rect-pen-down? rct)))
...
;; TESTS\EXAMPLES:
(begin-for-test
  (check-equal? (rect-change-velocity-x (new-rectangle 60 50 0 0) -)
                (new-rectangle 60 50 -2 0)
                "Velocity vx should become -2")
  (check-equal? (rect-change-velocity-x (new-rectangle 60 50 0 0) +)
                (new-rectangle 60 50 2 0)
                "Velocity vx should become 2")
  (check-equal? (rect-change-velocity-y (new-rectangle 60 50 0 0) -)
                (new-rectangle 60 50 0 -2)
                "Velocity vy should become -2")
  (check-equal? (rect-change-velocity-y (new-rectangle 60 50 0 0) +)
                (new-rectangle 60 50 0 2)
...
;; rect-pen-change : Rectangle Boolean-> Rectangle
;; GIVEN: Rectangle rct and Boolean boo as a pen status
;; RETURNS: Given rectangle with pen-down? for 'true' status, else pen-down? = false
;; STRATEGY: Use template for Rectangle on rct

(define (rect-pen-change rct boo)
  (make-rect
   (rect-x rct)
   (rect-y rct)
   (rect-vx rct)
...
(begin-for-test
  (check-equal?
   (rect-pen-change RECT_PEN_UP true)
   RECT_PEN_DOWN
   "Should return Rectangle with pen down")
  (check-equal?
   (rect-pen-change RECT_PEN_DOWN false)
   RECT_PEN_UP
   "Should return Rectangle with pen up")
  (check-equal?
   (rect-pen-change RECT_PEN_UP false)
   RECT_PEN_UP
...
(define (lor-after-mouse-event lor mx my mev)
  (map
   (lambda (rect) (rect-after-mouse-event (first lor) mx my mev))
   lor))
